    ...
    <div className="flex h-full flex-col gap-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex flex-col gap-6">
        ...
        <div className="space-y-5">
          ...
          {/* Outbound Phone Number / Region Dropdown (Commented out per request - can be re-enabled later) */}
          {/*
          <div className="flex flex-col gap-1.5">
            <label className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-[#111827] dark:text-zinc-100">
          ...
          */}
          ...
          <div className="flex flex-col gap-1.5 pt-1">
            ...
            {/* Contact Range Selection (Only shown for batch uploads with contacts) */}
            {formData.uploadSource !== "single" && (fileUploaded || (totalContacts !== undefined && totalContacts > 0)) && (
              <div className="mt-2 rounded-xl border border-zinc-200 bg-zinc-50/70 p-3.5 dark:border-zinc-800 dark:bg-zinc-800/40">
                <div className="flex items-center justify-between mb-2.5">
                  <label className="text-xs font-bold uppercase tracking-widest text-[#111827] dark:text-zinc-100 flex items-center gap-1.5">
                    <Users className="h-3.5 w-3.5 text-violet-500" />
                    Select Contacts Range
                  </label>
                  <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">
                    Total: {totalContacts || 0} contacts
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <button
                    type="button"
                    onClick={() => onChange({ selectionType: "all" })}
                    disabled={disabled}
                    className={`flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-bold transition ${
                      formData.selectionType === "all"
                        ...
                        : "bg-white text-zinc-700 hover:bg-zinc-100 border border-zinc-200 dark:bg-zinc-900 dark:text-zinc-300 dark:border-zinc-700"
                    }`}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                    All Contacts ({totalContacts || 0})
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      onChange({
                        selectionType: "range",
                        startRow: formData.startRow || 1,
                        endRow: formData.endRow || totalContacts || 1,
                      });
                    }}
                    disabled={disabled}
                    className={`flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-bold transition ${
                ...
                {formData.selectionType === "range" && (
                  <div className="space-y-2.5 pt-1">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 mb-1 block">
                          Start Row
                        </label>
                        <input
                          type="number"
                          ...
                          onChange={(e) => {
                            ...
                            onChange({ startRow: isNaN(val) ? 1 : val });
                          }}
                          disabled={disabled}
                          className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold transition focus:border-violet-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-900"
                        />
                      </div>
                      <div>
                        <label className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 mb-1 block">
                        ...
                        <input
                          ...
                          min={1}
                          max={totalContacts || 1}
                          value={formData.endRow ?? totalContacts ?? 1}
                          onChange={(e) => {
                            const val = parseInt(e.target.value, 10);
                            onChange({ endRow: isNaN(val) ? 1 : val });
                          }}
                          disabled={disabled}
                          className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold transition focus:border-violet-500 focus:outline-none dark:border-zinc-700 dark:bg-zinc-900"
                        />
                    ...
                    {/* Quick presets */}
                    {totalContacts && totalContacts > 10 && (
                      <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                        <span className="text-[10px] font-medium text-zinc-400">Quick presets:</span>
                        {[25, 50, 100, 250, 500].filter(n => n < (totalContacts || 0)).map((num) => (
                          <button
                            key={num}
                            type="button"
                            onClick={() => onChange({ startRow: 1, endRow: num })}
                            className="rounded-md border border-zinc-200 bg-white px-2 py-0.5 text-[10px] font-semibold text-zinc-600 hover:border-violet-400 hover:text-violet-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300"
                          >
                            First {num}
                          </button>
                        ))}
                        <button
                          type="button"
                          onClick={() => onChange({ startRow: 1, endRow: totalContacts })}
                          className="rounded-md border border-zinc-200 bg-white px-2 py-0.5 text-[10px] font-semibold text-zinc-600 hover:border-violet-400 hover:text-violet-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300"
                        >
                          All ({totalContacts})
                        </button>
                    ...
                    <div className="text-[11px] text-violet-700 dark:text-violet-300 bg-violet-50 dark:bg-violet-950/40 p-2.5 rounded-lg border border-violet-100 dark:border-violet-900/30 flex items-center justify-between">
                      <span>
                        Dialing <strong>{Math.max(0, Math.min(totalContacts || 0, formData.endRow || (totalContacts || 0)) - Math.max(1, formData.startRow || 1) + 1)}</strong> contacts (Rows {formData.startRow || 1} to {formData.endRow || totalContacts}).
                      </span>
                      <span className="font-semibold text-[10px] text-violet-500">
                        Table below updated ↓
                      </span>
                    </div>
                  </div>
